...
$sql = "SELECT * FROM doctor WHERE DoctorLocation = '".$search_area."' AND DoctorInformation = '".$search_param."'";
...
$result = $conn->query($sql);
    ...
    if($result->num_rows > 0)
    {
        ...
        $contact = $row["contactNo"];
            ...
            $doctor_data = $doctor_data . "<div style='display: flex; align-items: center;'>
            <img class='dctext1-icon' alt='' style='padding: 30px; margin: 20px 0px 0px 10px;position: relative' src='{$doctorimage}' />
            <div style='font-size: 14px;'>
              <p style='display: inline-block; margin-right: 10px;'>{$doctorname}</p>
              <p style='display: inline-block; margin-right: 10px;'>{$doctorinfo}</p>
       ...
       $data = '<b class="serviceheading2">No doctor found in your area</b>';
       $doctor_data="";
      }
  
  } else {
  
      $data = '<b class="serviceheading2">Bad Query</b>';
      $doctor_data="";
  }
